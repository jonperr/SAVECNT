...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ajuda_pagina:"):
        ...
        await query.message.reply_document(
            document=InputFile(arquivo, filename="contatos.vcf"),
            caption="📇 Aqui estão seus contatos no formato VCF!"
        )
        os.remove(arquivo)
        # Não deletar a mensagem original para manter o contexto
        return
        ...
        await query.message.reply_document(
            document=InputFile(arquivo, filename="contatos.csv"),
        ...
        await query.message.reply_document(
            document=InputFile(arquivo, filename="contatos.json"),
        ...
        try:
            ...
            # Enviar arquivos individualmente (o Telegram não permite enviar grupo de documentos mistos)
            await query.message.reply_document(
                document=InputFile(arquivos[0], filename="contatos.vcf"),
                caption="📇 Contatos em formato VCF"
            )
            await query.message.reply_document(
                document=InputFile(arquivos[1], filename="contatos.csv"),
                caption="📊 Contatos em formato CSV"
            )
            await query.message.reply_document(
                document=InputFile(arquivos[2], filename="contatos.json"),
